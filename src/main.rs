...
///returns a readable name for a specific device
...
fn App() -> Element {
    ...
    let mut hosted_pairing_code = use_signal(|| None::<String>);
    let mut peer_sync_is_busy = use_signal(|| false);
    ...
    rsx! {
                                        ...
                                        aside { class: "detail-column",
                                            ...
                                            if let Some(vn) = selected_vn {
                                                DetailView {
                                                    ...
                                                    hosted_pairing_code: hosted_pairing_code.read().clone(),
                                                    peer_sync_is_busy: *peer_sync_is_busy.read(),
                                                    on_host_sync: move |id| {
                                                        let selected_vn = vns
                                                            .read()
                                                            .iter()
                                                            .find(|vn| vn.id == id)
                                                        ...
                                                        let Some(vn) = selected_vn else {
                                                            notification.set(Some(AppNotification {
                                                                level: NotificationLevel::Error,
                                                                message: "Could not host sync: VN was not found.".to_string(),
                                                            }));
                                                            return;
                                                        };

                                                        if vn.save_sync.sync_id.is_empty() {
                                                            notification.set(Some(AppNotification {
                                                                level: NotificationLevel::Error,
                                                                message: "Could not host sync: this VN has no sync ID.".to_string(),
                                                            }));
                                                            return;
                                                        }
                                                        ...
                                                        let data_directory = match kakera_data_dir() {
                                                            ...
                                                            Err(error) => {
                                                                notification.set(Some(AppNotification {
                                                                    level: NotificationLevel::Error,
                                                                    message: format!("Could not host sync: {error}"),
                                                                }));
                                                                return;
                                                            }
                                                        };
                                                        ...
                                                        let device_id = match load_or_create_device_id(sync_directory.clone()) {
                                                            ...
                                                            Err(error) => {
                                                                notification.set(Some(AppNotification {
                                                                    level: NotificationLevel::Error,
                                                                    message: format!("Could not load this device's sync ID: {error}"),
                                                                }));
                                                                return;
                                                            }
                                                        };
                                                        ...
                                                        let mut host_notification = notification;
                                                            ...
                                                            let endpoint = match bind_peer_endpoint(sync_directory).await {
                                                                ...
                                                                Err(error) => {
                                                                    ...
                                                                    host_notification.set(Some(AppNotification {
                                                                        level: NotificationLevel::Error,
                                                                        message: format!("Could not start peer sync: {error}"),
                                                                    }));
                                                                    return;
                                                                }
                                                            };
                                                            ...
                                                            match transfer_result {
                                                                Ok(completed) => {
                                                                    host_notification.set(Some(AppNotification {
                                                                        ...
                                                                        message: format!("Sent the latest {vn_title} snapshot ({}) to the peer.", &completed.snapshot_id[..10])
                                                                    }));
                                                                }
                                                                ...
                                                                Err(error) => {
                                                                    host_notification.set(Some(AppNotification {
                                                                        level: NotificationLevel::Error,
                                                                        message: format!("Peer sync failed for {vn_title}: {error}"),
                                                                    }));
                                                                }
                                                            }
                                                        });
                                                    ...
                                                    on_receive_snapshot: move |(id, pairing_code): (u64, String)| {
                                                        ...
                                                        let Some(vn) = selected_vn else {
                                                            notification.set(Some(AppNotification {
                                                                level: NotificationLevel::Error,
                                                                message: "Could not receive snapshot: VN was not found.".to_string(),
                                                            }));
                                                            return;
                                                        };
                                                        ...
                                                        let data_directory = match kakera_data_dir() {
                                                            ...
                                                            Err(error) => {
                                                                notification.set(Some(AppNotification {
                                                                    level: NotificationLevel::Error,
                                                                    message: format!("Could not receive snapshot: {error}"),
                                                                }));
                                                                return;
                                                            }
                                                        };
                                                        ...
                                                        let mut busy_state = peer_sync_is_busy;
                                                        let mut host_code_state = hosted_pairing_code;
                                                        let mut receive_notification = notification;
                                                        let mut library_state = vns;
                                                        let mut snapshot_state = sync_snapshots;
                                                        let selected_id_state = selected_vn_id;
                                                        ...
                                                        spawn(async move {
                                                            ...
                                                            match transfer_result {
                                                                Ok(received) => {
                                                                    let received_manifest = received.manifest.clone();
                                                                    let saved_sync_id = shared_sync_id.clone();

                                                                    update_vn_and_save(&mut library_state, id, move |vn| {
                                                                        vn.save_sync.sync_id = saved_sync_id;
                                                                    }, "Could not save the paired sync id".to_string());

                                                                    if *selected_id_state.read() == Some(id) {
                                                                        let already_visible = snapshot_state
                                                                            .read()
                                                                            .iter()
                                                                            .any(|snapshot| {
                                                                                snapshot.snapshot_id == received_manifest.snapshot_id
                                                                            });

                                                                        if !already_visible {
                                                                            snapshot_state
                                                                                .write()
                                                                                .insert(0, received_manifest.clone());
                                                                        }
                                                                    }
                                                                ...
                                                                Err(error) => {
                                                                    receive_notification.set(Some(AppNotification {
                                                                        level: NotificationLevel::Error,
                                                                        message: format!("Could not receive snapshot for {vn_title}: {error}")
                                                                    }));
                                                                }
                                                            }
                                                        });
        ...
        Ok(ToolLaunch::Queued) => {
                                     ...
                                     Some(AppNotification {
                                                    ...
                                                    //when changing steam app id
                                                    on_steam_app_id_change: move |(id, app_id_text): (u64, String)| {
                                                        let trimmed_app_id = app_id_text.trim();

                                                        let steam_app_id = if trimmed_app_id.is_empty() {
                                                            None
                                                        } else {
                                                            match trimmed_app_id.parse::<u32>() {
                                                                Ok(app_id) if app_id > 0 => Some(app_id),
                                                                _ => {
                                                                    notification.set(Some(AppNotification {
                                                                        level: NotificationLevel::Error,
                                                                        message: "Steam App ID must be a positive whole number.".to_string(),
                                                                    }));
                                                                    return;
                                                                }
                                                            }
                                                        ...
                                                        update_vn_and_save(
                                                            ...
                                                            id,
                                                            move |vn| {
                                                                vn.steam_app_id = steam_app_id;
                                                            },
                                                            "Could not save Steam App ID".to_string(),
                                                        );
                                                    },
                               ...
                               id,
                                                    ...
                                                    on_create_snapshot: move |id| {
                                                        ...
                                                        let Some(vn) = selected_vn else {
                                                            notification.set(Some(AppNotification {
                                                                level: NotificationLevel::Error,
                                                                message: "Could not create snapshot: VN was not found.".to_string(),
                                                            }));
                                                            return;
                                                        };
                                                        ...
                                                        if !vn.save_sync.enabled || vn.save_sync.locations.is_empty() {
                                                            notification.set(Some(AppNotification {
                                                                level: NotificationLevel::Warning,
                                                                message: "Enable save sync and add a save location first.".to_string(),
                                                            }));
                                                            return;
                                                        }
                                                        ...
                                                        let data_directory = match kakera_data_dir() {
                                                            ...
                                                            Err(error) => {
                                                                notification.set(Some(AppNotification {
                                                                    level: NotificationLevel::Error,
                                                                    message: format!("Could not create snapshot: {error}")
                                                                }));
                                                                return;
                                                            }
                                                        };
                                                        ...
                                                        let vn_title = vn.title.clone();
                                                        let locations = vn.save_sync.locations.clone();
                                                        let mut snapshot_notification = notification;
                                                            ...
                                                            match task_result {
                                                                Ok(Ok(snapshot)) => {
                                                                    ...
                                                                    snapshot_notification.set(Some(AppNotification {
                                                                        level: NotificationLevel::Info,
                                                                        message: format!(
                                                                            "Created snapshot for {vn_title}: {} files, {} new blobs, {} reused.",
                                                                            snapshot.manifest.files.len(),
                                                                            snapshot.new_blob_count,
                                                                ...
                                                                Ok(Err(error)) => {
                                                                    snapshot_notification.set(Some(AppNotification {
                                                                        level: NotificationLevel::Error,
                                                                        message: format!("Could not create snapshot for {vn_title}: {error}")
                                                                    }));
                                                                }
                                                                ...
                                                                Err(error) => {
                                                                    snapshot_notification.set(Some(AppNotification {
                                                                        level: NotificationLevel::Error,
                                                                        message: format!("Snapshot task failed for {vn_title}: {error}")
                                                                    }));
                                                                }
                                                            }
                                                        });
